...
> **Bosak baseline:** 18,516 passed / 3,503 failed / 9,802 skipped (W3C QT3 conformance)
...
### Known gaps
- `fn:sort` — mixed-type comparator issues (20+ failures)
- `fn:function-lookup` — double-to-string precision mismatches
- JSON/XML functions — `json-to-xml`, `parse-json`, `xml-to-json`, `transform`, `load-xquery-module` not implemented
- Date/time ordering (`lt`, `gt`) — 9 remaining failures
- `fn:path` — 6 remaining syntax-level parser failures
...
**W3C QT3 conformance:** 18,516 passed / 3,503 failed / 9,802 skipped  
