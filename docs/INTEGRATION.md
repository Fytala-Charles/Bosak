...
> **Bosak baseline:** 1,786 unit tests passed / 0 failed / 0 skipped
> **QT3 baseline:** **30,821 passed / 52 failed / 948 skipped** (96.86%). The schema-derived string/numeric/union cast cluster is fixed: derived string subtypes (`xs:normalizedString`, `xs:token`, etc.) now accept numeric input, derived atomic pattern-facet validation uses XSD canonical lexical forms, list type casts reject single non-string atomic values, and `TimeSpan`-backed durations serialize back to XSD duration lexical form. `XdmValue.ToString()` respects `gYear`/`gYearMonth`/`gMonth`/`gMonthDay`/`gDay` schema type annotations. The conformance runner skips `app-Demos`, `app-XMark`, and `cbcl-codepoints-to-string-021` so unattended full sweeps complete reliably.
