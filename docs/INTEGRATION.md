...
> **QT3 baseline:** **30,854 passed / 19 failed / 948 skipped** (96.96%). The namespace-sensitive atomic function-conversion cluster is fixed: `ApplyFunctionConversion` now raises `XPTY0117` when `xs:untypedAtomic` is supplied to `xs:QName`, `xs:NOTATION`, or a user-defined restriction of those, before subtype substitution can silently accept it. This closes `CastAs675a`, `CastAsNamespaceSensitiveType-1`, and `CastAsNamespaceSensitiveType-2`. The schema-aware list/union cluster remains regression-tested; the castable cluster and conformance-runner skips (`app-Demos`, `app-XMark`, `cbcl-codepoints-to-string-021`) remain in place.
