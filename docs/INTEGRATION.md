...
> **Bosak baseline:** 1,792 unit tests passed / 0 failed / 0 skipped
> **QT3 baseline:** **30,832 passed / 41 failed / 948 skipped** (96.89%). The schema-validated date/time timezone preservation cluster is fixed: `XDocumentNode.GetTypedValue` re-parses the lexical string for `xs:date`, `xs:time`, `xs:dateTime`, `xs:dateTimeStamp`, and the `g*` date/time types using `XmlConvert.ToDateTimeOffset`, preserving explicit offsets such as `+05:00` and `Z`. This closes `prod-CastExpr.schema` `casthcds30`–`casthcds34` and `prod-WindowClause` `WindowingUseCase*`. The earlier schema-derived string/numeric/union cast cluster remains fixed, and the conformance runner still skips `app-Demos`, `app-XMark`, and `cbcl-codepoints-to-string-021` so unattended full sweeps complete reliably.
