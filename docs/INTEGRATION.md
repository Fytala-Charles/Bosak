...
> **Bosak baseline:** 1,806 unit tests passed / 0 failed / 0 skipped
> **QT3 baseline:** **30,842 passed / 31 failed / 948 skipped** (96.92%). The op-numeric-add / union named-member cluster is fixed: `XPathParser.ParseSingleType` disambiguates `*`/`+` after a cast/castable target type, and `VmEngine.GetUnionMemberTypes` now returns both inline and named `@memberTypes` union members. This closes `op-numeric-add-13`–`op-numeric-add-16` and additional union/cast residuals. The castable cluster remains fixed, and the conformance runner still skips `app-Demos`, `app-XMark`, and `cbcl-codepoints-to-string-021` so unattended full sweeps complete reliably.
