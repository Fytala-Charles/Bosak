...
> **Bosak baseline:** 1,801 unit tests passed / 0 failed / 0 skipped
> **QT3 baseline:** **30,836 passed / 37 failed / 948 skipped** (96.90%). The castable cluster is fixed: `VmEngine.TryCast`, the `Cast` opcode, and the `Castable` opcode now share an `AtomizeForCast` helper that recursively atomizes arrays, atomizes nodes, and raises `FOTY0013` for maps and function items; `castable as` propagates type errors instead of returning `false`. This closes `prod-CastableExpr` `CastableAs665`–`CastableAs668`. The earlier schema-validated date/time timezone preservation and schema-derived string/numeric/union cast clusters remain fixed, and the conformance runner still skips `app-Demos`, `app-XMark`, and `cbcl-codepoints-to-string-021` so unattended full sweeps complete reliably.
