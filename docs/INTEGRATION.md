...
- **2026-09-10** — **Performance wave 3 (REQ-085):** serializer path — HTML escaping no longer allocates per-character strings or performs per-character encoding lookups (clean spans are written whole; unicode encodings skip representability checks), and namespace bindings during HTML serialization are copy-on-write instead of two dictionary copies per element. **Transform_HtmlTable 73.06 → 60.14 ms (cumulative 193.34 → 60.14, −69%), 71.53 → 60.20 MB (cumulative 115.48 → 60.20, −48%)**; output bytes unchanged (XSLT strict sweep 7,722/3/6,875, QT3 31,142/0/679, unit 2,216/0/0).
