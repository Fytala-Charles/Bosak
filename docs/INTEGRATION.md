...
> **Bosak baseline:** 1,813 unit tests passed / 0 failed / 0 skipped
> **QT3 baseline:** **30,851 passed / 22 failed / 948 skipped** (96.95%). The schema-aware list/union function-conversion cluster is regression-tested: attribute kind tests preserve the case of the user-defined type name; union function conversion applies membership semantics, casts `xs:untypedAtomic` to the first matching non-namespace-sensitive member, rejects non-matching values with `XPTY0004`, and rejects namespace-sensitive unions with `XPTY0117`; unprefixed user-defined schema types are accepted in `instance of` via the default element namespace; and `element(*, T1)` / `attribute(*, T1)` subtyping walks the schema type hierarchy. Targeted verification: `prod-FLWORExpr` 21/0/28; `prod-FunctionCall` 120/0/32. The castable cluster and conformance-runner skips (`app-Demos`, `app-XMark`, `cbcl-codepoints-to-string-021`) remain in place.
