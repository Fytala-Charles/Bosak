...
1. **Implemented fragment-identifier support for embedded stylesheet modules** —
   - `Stylesheet.ResolveImport` and `Stylesheet.ResolveInclude` now detect `#fragment` references in `@href`.
   - Added helper `ExtractModuleDocument` that loads the containing document and extracts the element identified by `xml:id` or plain `id`.
   - The extracted element is wrapped in a new document whose base URI is the source document URI, preserving any `xml:base` attributes inside the embedded module (include-0103).
   - Missing fragments raise `XTSE0165`.
   - Header bumped: `Stylesheet.cs` → 2.79.

2. **Enabled the previously skipped `include-0102` and `include-0103` tests** —
   - Removed `include-0102` and `include-0103` from the harness skip list in `tests/Bosak.Xslt.Conformance/Program.cs`.
   - Header bumped: `Program.cs` → 3.26.

3. **Added regression tests** —
   - `Include_WithPlainIdFragment_ExtractsEmbeddedStylesheet`
   - `Include_WithXmlIdFragment_ExtractsEmbeddedStylesheet`
   - `Include_WithMissingFragment_ThrowsXtse0165`
   - `Import_WithFragmentIdentifier_ExtractsEmbeddedStylesheet`
   - Header bumped: `StylesheetTests.cs` → 0.81.

4. **Results** —
   - `include` test set: **16 passed / 0 failed / 0 skipped** (was 14/0/2).
   - Unit tests: **2,083 passed / 0 failed / 0 skipped**.
   - Routine XSLT sweep: **7,092 passed / 0 failed / 7,508 skipped** (was 7,090/0/7,510; +2 runnable tests).
...
1. **Detect conflicting `input-type-annotations` across modules (XTSE0265)** —
   ...
   - `Stylesheet.ValidateInputTypeAnnotations` recursively inspects the root stylesheet and all imported/included modules. If any two modules declare different non-`unspecified` values, the compilation raises `XTSE0265`.
   - Matching values or one `unspecified` module combined with an explicit value remain valid.
   - The W3C `error-0265*` cluster passes with 1/0/0.
   - Added 3 regression tests in `tests/Bosak.Xslt.Tests/StylesheetTests.cs` for conflict, matching values, and unspecified-with-explicit.
   - Header bumped: `Stylesheet.cs` → 2.71, `StylesheetTests.cs` → 0.67.

2. **Results** —
   - `error-0265*` cluster: 1 passed / 0 failed / 0 skipped.
   - Routine XSLT sweep remains **7,056 passed / 0 failed / 7,544 skipped**.
   - Unit tests: **2,019 passed / 0 failed / 0 skipped**.

## Previous Session Changes (XTSE1600 cluster)
...
1. **`fn:current()` without current item raises `XTDE1360`** —
   ...
   - `TransformEngine.ExecuteXsltFunction` clears the current item before evaluating a stylesheet-function body, so `fn:current()` inside an `xsl:function` raises `XTDE1360`.
   - `VmEngine.InvokeFunctionItem` clears the current item for the duration of a dynamic function-item call, so `fn:current()` invoked through a function reference such as `current#0` raises `XTDE1360`.
   - The W3C `error-1360*` cluster passes with 2/0/0.
   - Added regression tests in `tests/Bosak.Xslt.Tests/StylesheetTests.cs` for a valid `current()` inside `xsl:for-each`, `current()` inside `xsl:function`, and `current()` through a function item.
...
3. **Results** —
   ...
   - `error-0870*`, `error-0840*`, `error-3190*` targeted clusters remain green.
   - Routine XSLT sweep remains **7,056 passed / 0 failed / 7,544 skipped**.
   - Unit tests: **1,982 passed / 0 failed / 0 skipped**.

## Previous Session Changes (XTDE1428 cluster)
