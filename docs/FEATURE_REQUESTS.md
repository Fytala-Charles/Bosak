...
| 2026-09-05 | Kimi | Sponsorship layer drafted (item 9 — pending user enrollment) | Decision: GitHub Sponsors on the **`Fytala-Charles` account** (the repo-owning public identity — sponsors land on the Fytala-branded profile carrying the Bosak + youth-technology mission; mechanics identical for any personal account). Added `.github/FUNDING.yml` (`github: [Fytala-Charles]`); the button activates once that account enrolls at github.com/sponsors/accounts. Sponsor benefits per the checklist: roadmap votes + priority triage; core conformance fixes are never gated behind payment. Charles to complete enrollment (2FA, bank/PayPal, tax form, profile text) and choose tiers. Remaining: 10 commercial boundary ratification. |
