...
- Predicate-path intermediate lists (~1 KB per node-predicate on the predicated benchmark).
- XSLT result-tree construction + serialization (Transform_HtmlTable is now the largest benchmark: 198 ms / 103 MB).
- Per-transform dedicated-thread spawn reuse (XsltExecutable.RunWithStack).
- FLWOR tuple materialization for large result sets.
