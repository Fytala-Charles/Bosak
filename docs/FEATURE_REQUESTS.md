...
| 4 | TBD | **Streaming** | Phases A+B implemented (2026-09-16) | A: burst-mode streaming input (`XmlStreamingProvider`, `TransformStreaming`, VM/XSLT single-pass paths, bounded memory at 500k records). B: push-style streaming accumulators — per-record values as annotations, drain-on-read at doc/root, on-demand after-resolution, deferred rule errors; `fn:snapshot` supports streamed nodes; W3C `decl/accumulator` 93/0/14 (100% runnable); XSLT sweep 7,759/3/6,838. C pending: `streamable="yes"`, §19 posture analysis, `strm/` sets; `xsl:supports-streaming` still reports `no`. |
