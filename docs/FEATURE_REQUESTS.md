...
| 2 | TBD | **XSLT 3.0 packages** (`xsl:package`, `xsl:use-package`) | In Progress | Package root parsing, use-package resolution, accept/override visibility for functions/variables/parameters, per-package lazy-global isolation, and package-version range matching (`PackageVersion`, `VersionMatches`, `ResolvePackageLocation`, `PackageVersionResolutionStrategy`) implemented; remaining scope is secondary-package harness registration. |
...
| 2026-09-05 | Kimi | Sponsorship layer drafted (item 9 — pending user enrollment) | Decision: GitHub Sponsors on the **personal account `poco-irrilevante`**, not the `Fytala-Charles` repo account — Sponsors requires a personal account, and the Bosak repo will display the funding button regardless of which account owns the repo. Added `.github/FUNDING.yml` (`github: [poco-irrilevante]`); the button activates once the personal account enrolls at github.com/sponsors/accounts. Sponsor benefits per the checklist: roadmap votes + priority triage; core conformance fixes are never gated behind payment. Charles to complete enrollment (2FA, bank/PayPal, tax form, profile text) and choose tiers. Remaining: 10 commercial boundary ratification. |
