...
| 4 | TBD | **Streaming** | Phase A implemented (2026-09-16) | Burst-mode streaming input: `XmlStreamingProvider` (`Bosak.XPath.Providers.Streaming`) presents an XmlReader source as a lazy document; VM/XSLT single-pass paths keep record-at-a-time processing in bounded memory (verified at 500k records); `XsltExecutable.TransformStreaming` API. Phase B (streaming accumulators) and Phase C (`streamable="yes"`, §19 posture analysis) pending; `xsl:supports-streaming` still reports `no`. |
