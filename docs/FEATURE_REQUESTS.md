...
1. **`Bosak.LanguageServer`** — .NET 10 console app using OmniSharp.Extensions.LanguageServer 0.19.9:
   ...
   - `CodeActionHandler`: quick fixes for undeclared namespace prefixes in XQuery/XSLT, promotion of bare `<stylesheet>`/`<transform>` roots to `xsl:*`, and missing `version` attribute on `xsl:stylesheet`/`xsl:transform`
...
- [x] Code actions offer quick fixes for undeclared namespace prefixes, missing XSLT namespace, and missing XSLT version attribute
