...
> **Living Registry** — Last updated: 2026-06-27 (`maps` cluster fully cleared; `mode`, `static`, and `next-match` regressions fixed; `arrays-301`–`305` now pass; `namespace`, `namespace-alias`, `date`, `call-template`, `attribute`, `system-property`, `unparsed-text-lines`, `regex`, and `mode` clusters green; full W3C baseline 4,799/451/9,350)    
