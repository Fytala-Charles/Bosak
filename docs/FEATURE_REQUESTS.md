...
> **Living Registry** — Last updated: 2026-08-20 (Schema awareness sweep closed: user-defined schema simple-type constructors registered from `EvaluationContext.SchemaSet`; `ValueMatchesType`, `ApplyFunctionConversion`, and `instance of` now handle prefixed user-defined schema types; schema-validated typed values preserve the integer XDM kind for integer-derived types and keep date/time values typed; full QT3 sweep 29,929/0/1,892 with `cbcl-schema-element/attribute` kind tests, list/union types, and QName/NOTATION casts documented as known gaps; unit tests pass 1,708/0) (`.xq`/`.xqy`/`.xquery`) — highlighting, diagnostics, completion, hover, go-to-definition, document symbols, and a Run XQuery command; language server now supports hover, go-to-definition, document symbols, and working evaluate-XPath / transform-XSLT commands for XSLT/XPath; new `Bosak.LanguageServer.Tests` project; XML 1.1 namespace undeclaration: element constructors accept `xmlns:p=""` in XML 1.1 mode and record a `PrefixedNamespaceUndeclarations` annotation instead of raising XQST0085; `XQST0085b`, `K2-Serialization-20/21` pass; the two XML 1.1-only character-name tests are documented as a known gap; QT3 30,344/0/1,477; QT3 skip-cluster cleanup — **external variable declared-type cluster**: main-module function bodies use the main module's static default element namespace; `extvardeclwithtype-23` passes; **group-by post-clause cluster**: `let` between `group by` and `order by` is evaluated during re-keying; `for`/`window` after `group by` lowered as nested FLWOR; `use-case-groupby-Q6` and `TumblingWindowExpr545` pass; `prod-GroupByClause` 36/0/0; **multiple order by cluster**: FLWORs support multiple `order by` clauses via stable-sort re-key stages; `orderBy65`/`orderBy66` pass; `prod-OrderByClause` 201/0/4; **attribute whitespace/namespace cluster**: direct attribute values apply XML 1.0 line-ending normalization before whitespace normalization; unprefixed computed attribute names ignore the default element namespace; `K2-DirectConElemAttr-75` and `currencysvg` pass; **xml:space computed-attribute cluster**: `ConstructAttribute` validates `xml:space` and raises `XQDY0092` for invalid values; `K2-ComputeConAttr-60` passes; **UCA identical/blanked cluster**: `fn:compare` with `strength=identical;alternate=blanked` uses a codepoint tie-break instead of combining `CompareOptions.Ordinal` with `IgnoreSymbols`; `compare-042` passes; **fn-transform cluster**: missing source documents raise `FOXT0002`; secondary result-document text content is captured via the `__xdm_doc__` wrapper; `fn-transform` 120/0/4; **arrow partial-application cluster**: `LowerArrow` supports placeholder arguments in static arrow calls such as `"$" => concat(?)`; `prod-ArrowPostfix` 42/0/0; **function-arity overflow cluster**: huge named-function-ref arity literals clamp to `int.MaxValue` and resolve as `FOAR0002`; `fn-function-arity-017` and `fn-function-name-018` now pass; full sweep completes at 30,329/0/1,492 (95.31%) after skipping the `app-CatalogCheck` catalog-consistency set; **misc-JsonTestSuite cluster**: `JsonDoc` resolves relative URIs against `EvaluationContext.BaseUri` and reads local JSON files as plain text; all 318 previously skipped `misc-JsonTestSuite` tests now pass; targeted verification `misc-JsonTestSuite` 318/0/0; **fn-json-doc cluster**: `FunctionLibrary.JsonDoc` wraps `EvaluationContext.DocumentLoader` failures as `FOUT1170`; 5 previously skipped `json-doc-error-028..032` tests now pass; targeted verification `fn-json-doc` 61/0/7; **group-by/order-by cluster**: `IrLowerer` supports `where` and `let` clauses after `group by` and `order by`; post-grouping `let` variables referenced by post-grouping `order by` keys are rejected cleanly as unsupported; targeted verification `prod-GroupByClause` 35/0/1, `prod-OrderByClause` 199/0/6, `app-Duplicates` 14/0/0; 1 former failure converted to skip; 4 unit regression tests added; document-node stripping cluster: `XDocumentProvider.ConstructElement` and `ConstructDocument` strip document nodes used as element/document content, unwrap the synthetic `__xdm_doc__` wrapper, and merge adjacent text children so ~17 previously skipped tests now pass; `<test file="..."/>` queries now loaded from external `.xq` files; reserved namespace mistakes raise `XQST0070`/`XQDY0096`/`XQST0085`/`XQDY0101` spec codes instead of raw .NET `ArgumentException`; 43 skips converted to passes; 6 new KnownXQueryGaps documented; QT3 residual singles/pairs sweep — **if-keyword-as-name cluster**: `XPathParser.ParseExprSingle` treats `if` as a conditional keyword only when followed by `(`; otherwise `if` is parsed as an ordinary name/name test, fixing the tokenizer-torture query `if(if) then then else else-...` (K2-NameTest-5); 1 stale KnownXQueryGaps removed; QT3 residual singles/pairs sweep — **distinct-values cluster**: fn:distinct-values/index-of distinguish XSD string type families (gYear/gMonth/gDay/gYearMonth/gMonthDay, hexBinary, base64Binary) from xs:string/untypedAtomic/anyURI and derived string types; cbcl-distinct-values-002b passes, 1 stale KnownXQueryGaps removed; prior sweep wave 3 — function items, constructors, validation, environment variables; engine sweep: named function-item subtyping with kind-derived signatures, FOTY0013 on function comparisons, array atomization in function conversion, fn:filter xs:boolean conversion, fn:parse-xml(()) empty sequence, namespace-uri-for-prefix xs:anyURI, empty enclosed expressions, whitespace between constructor attributes, XML 1.0 line-ending normalization, strict-validation whitespace stripping, switch-case NaN equality, subscript int-range guard; **collation cluster**: order-by clauses use effective collation, fn:sort/array:sort fall back to default collation, default collation declaration stores resolved URI, 8 stale KnownXQueryGaps removed; **deep-equal cluster**: fn:deep-equal ignores comments and processing instructions in element children, 5 stale KnownXQueryGaps removed; **direct constructor cluster**: attribute value constructors include comment/PI string values, xml:space validation raises XQDY0092, 3 stale KnownXQueryGaps removed; **map:merge cluster**: default duplicates option is use-first per F+O 3.1, 5 stale KnownXQueryGaps removed; **query-based collections cluster**: QT3 harness evaluates environment <collection><query> declarations into EvaluationContext.CollectionValues, fn:collection/fn:uri-collection honor precomputed values, 6 stale KnownXQueryGaps removed; **NameTest cluster**: document-node(element(...)) instance-of checks preserve nested kind-test case (NodeTest004), 1 stale KnownXQueryGaps removed; **UseCaseR31 cluster**: map dynamic calls return empty sequence for missing keys, maps/arrays coerce to typed function items, 2 stale KnownXQueryGaps removed; **date/time extraction cluster**: fn:*-from-dateTime/date/time declare ParameterTypeNames so nodes are atomized and cast to xs:dateTime/xs:date/xs:time, 1 stale KnownXQueryGaps removed (rdb-queries-results-q9); **assert-eq cluster**: unwrap singleton sequences before comparing so single-item QName sequences compare correctly (fn-node-name-26), 1 stale KnownXQueryGaps removed; **fn:path cluster**: GetXPathParent returns the owning XDocument for document-level PIs/comments so fn:path emits /processing-instruction(...)[n], with a restricted fallback to avoid an XDocument self-loop (path009), 1 stale KnownXQueryGaps removed; **unparsed-text-available cluster**: fn:unparsed-text-available#2 raises XPTY0004 when $encoding is empty (fn-unparsed-text-available-012), 1 stale KnownXQueryGaps removed; **assert-xml cluster**: NormalizeXml strips trailing whitespace after the last element in multi-root fragments (d1e74610), 1 stale KnownXQueryGaps removed; QT3 **29,936 passed / 0 failed / 1,885 skipped** (94.07%); unit tests **1,695/0**); **analyze-string cluster**: fn:analyze-string result element declares fn namespace explicitly so in-scope-prefixes reports fn and xml, 1 stale KnownXQueryGaps removed (analyzeString-028); **namespace-fixup cluster**: XQuery element constructors allocate generated prefixes and annotate attributes when copied attributes share a prefix bound to different URIs, 1 stale KnownXQueryGaps removed (cbcl-ns-fixup-1); **axis-step cluster**: ApplyAxis/PathStepMap treat empty-sequence input (e.g. doc(())/*) as empty instead of XPDY0002, Catalog004 passes, 1 stale KnownXQueryGaps removed
...
- [x] Code actions offer quick fixes for XPath syntax errors, undeclared namespace prefixes (including XPST0081 diagnostic-driven fixes), XQuery `import module namespace`, invalid XQuery empty namespace declarations (XQST0085), missing XSLT namespace, and missing XSLT version attribute
...
| 3 | REQ-070 | **Schema awareness / XSD validation** | In Progress | User-defined schema simple-type constructors, cast/instance-of matching, typed-value integer preservation; remaining: `schema-element()`/`schema-attribute()` kind tests (XDocument provider limitation). |
