...
> **Living Registry** — Last updated: 2026-08-21 (dynamic constructor calls now capture/restore the static namespace context for namespace-sensitive union constructors; restrictions of union/list types are rejected as SequenceType item types with XPST0051; also fixes CastAs-UnionType-13/14/15/17; `prod-CastExpr.schema` 117/12/1, `prod-CastableExpr UnionType` 29/0, `prod-CastableExpr ListType` 18/0; unit tests pass 1,762/0)
