...
| 2 | — | **XSLT 3.0 packages** | ✅ Implemented (1 known residual) | `xsl:package` root parsing, `xsl:use-package` resolution to registered packages, `xsl:expose` static validation and runtime visibility, `xsl:accept` visibility enforcement and runtime checks, accept/override visibility for functions/variables/parameters, diamond-import conflict handling, per-package lazy-global isolation, package-version range matching, `xsl:use-package` in imported modules (`XTSE3008`), library-package absent-focus globals (`XPDY0002`), and `xsl:original` for attribute-set overrides are implemented. `ResolvePackageLocation` uses `PackageVersion.Parse`, `VersionMatches`, and `PackageVersionResolutionStrategy` (`Highest`/`Lowest`). W3C clusters: `package` 159/1/3, `accept` 50/0/0, `expose` 42/0/0, `use-package` 53/0/1. Residual: `package-101` still fails because variable/function overrides from `xsl:override` are not yet visible inside used-package components that reference them. |
...
| XSLT 3.0 packages | ✅ Implemented (1 residual) | `xsl:package` root parsing, `xsl:use-package` resolution to registered packages, `xsl:expose` static validation and runtime visibility, `xsl:accept` visibility enforcement and runtime checks, accept/override visibility for functions/variables/parameters, CollectingScope-aware global collection and conflict validation, diamond-import same-name public variables, per-package lazy-global isolation, package-version range matching, `xsl:use-package` in imports/includes (`XTSE3008`), library-package absent-focus globals (`XPDY0002`), and `xsl:original` for attribute-set overrides are implemented. W3C clusters: `package` 159/1/3, `accept` 50/0/0, `expose` 42/0/0, `use-package` 53/0/1; `ResolvePackageLocation` selects the highest matching version by default. Residual: `package-101` still fails because `xsl:override` variables/functions are not visible inside used-package components that reference them. |
