...
| 4 | 5 | **Streaming** | ✅ Phases A+B+C (C1–C3) | Phase A (burst-mode input): `XmlStreamingProvider` + `XsltExecutable.TransformStreaming`, record-at-a-time processing in bounded memory; VM/XSLT single-pass paths gated on `ISinglePassSequence`. Phase B: push-style streaming accumulators (per-record values as annotations, drain-on-read, deferred rule errors). Phase C1–C3 (2026-09-17): `xsl:supports-streaming` = "yes"; streamable `xsl:source-document` implemented; compile-time XTSE3430 streamability analyzer (`StreamabilityAnalyzer`) — posture/sweep analysis per §19 with Saxon-compatible extensions (§19.10 striding unions, buffered-item leaf model), calibrated against all 67 targeted `strm/` sets; harness XTSE3430 skip removed with zero genuine regressions vs C1 per-set baselines; 106/113 corpus static-error cases caught (3 accepted non-catches: `si-fork-116`, `si-fork-901/902` schema artifacts); final full sweep **9,929/344/4,327**; W3C `decl/accumulator` **102/0/5**. Residual runtime forward-only consumption gaps (~143 failures) are Phase D material. |
...
| Streaming | ✅ Phases A+B+C (C1–C3) | Burst-mode streaming input via `XsltExecutable.TransformStreaming` + `XmlStreamingProvider` (record-at-a-time, bounded memory; forward-only root children with loud errors) and push-style streaming accumulators (per-record values as annotations; W3C `decl/accumulator` 102/0/5). Phase C1–C3: `xsl:supports-streaming` = "yes", streamable `xsl:source-document`, and XTSE3430 streamability analysis per §19 (`StreamabilityAnalyzer`, fail-open wrapper in `Stylesheet`) — the harness XTSE3430 expected-error cases run for real (skip removed, zero genuine regressions vs C1 per-set baselines, full sweep 9,929/344/4,327) |
