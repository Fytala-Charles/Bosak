...
|----------|-------|-------------|--------|-------|
| 1 | 4 | **XQuery 3.1** | 🚧 In Progress — Phase 4 | Full core FLWOR (`order by`/`count`/`group by`/`window`), direct and computed constructors with constructor-local namespaces, switch/typeswitch, output declarations and serialization, user-defined functions and variables (`declare function`/`declare variable` with function-item coercion), library modules (`module namespace`/`import module` with %public/%private visibility), try/catch with named error codes and `err:*` variables, string constructors, ordering features (`ordered`/`unordered`, ordering declarations), spec-correct name tests and constructor in-scope namespaces; QT3 harness routes supported XQuery tests (29,264/0, 91.96%). Remaining: VarDecl.external, Annotation, fn:load-xquery-module clusters. |
