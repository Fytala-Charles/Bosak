...
| 4 | 5 | **Streaming** | ✅ Phases A+B | Phase A (burst-mode input): `XmlStreamingProvider` + `XsltExecutable.TransformStreaming`, record-at-a-time processing in bounded memory; VM/XSLT single-pass paths gated on `ISinglePassSequence`. Phase B: push-style streaming accumulators (per-record values as annotations, drain-on-read, deferred rule errors); W3C `decl/accumulator` 93/0/14. Phase C pending: `streamable="yes"`, §19 posture analysis, `strm/` sets. |
...
| Streaming | ✅ Phases A+B | Burst-mode streaming input via `XsltExecutable.TransformStreaming` + `XmlStreamingProvider` (record-at-a-time, bounded memory; forward-only root children with loud errors) and push-style streaming accumulators (per-record values as annotations; W3C `decl/accumulator` 93/0/14). Phase C (`streamable="yes"`, §19 posture analysis) pending; `xsl:supports-streaming` reports `no` |
