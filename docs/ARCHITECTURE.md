...
| Streaming | ✅ Phases A+B+C+D | Burst-mode streaming input via `XsltExecutable.TransformStreaming` + `XmlStreamingProvider` (record-at-a-time, bounded memory by default; forward-only root children with loud errors) and push-style streaming accumulators (W3C `decl/accumulator` 102/0/5). Phase C: `xsl:supports-streaming` = "yes", streamable `xsl:source-document`, XTSE3430 streamability analysis per §19 (`StreamabilityAnalyzer`, fail-open wrapper in `Stylesheet`). Phase D: analyzer-accepted constructs execute — fused single-pass eager helpers, §11.7.3 content semantics, `fn:snapshot` grounding, opt-in record retention (tee/replay) for crawling multi-operand shapes, streaming DTD support. Group context is isolated across template invocation; duplicate map-constructor keys inside xsl:fork raise XTDE3365. Full sweep 10,164/111/4,325 |
