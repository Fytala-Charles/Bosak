...
| 2 | — | **XSLT 3.0 packages** | ✅ Implemented | `xsl:package` root parsing, `xsl:use-package` resolution to registered packages, accept/override visibility for functions/variables/parameters, diamond-import conflict handling, per-package lazy-global isolation, and package-version range matching are implemented. `ResolvePackageLocation` uses `PackageVersion.Parse`, `VersionMatches`, and `PackageVersionResolutionStrategy` (`Highest`/`Lowest`). All runnable W3C `use-package` tests pass. |
...
| XSLT 3.0 packages | ✅ Implemented | `xsl:package` root parsing, `xsl:use-package` resolution to registered packages, accept/override visibility for functions/variables/parameters, CollectingScope-aware global collection and conflict validation, diamond-import same-name public variables, per-package lazy-global isolation, and package-version range matching are implemented. `use-package-001` through `use-package-212` are runnable (53/54 pass, 1 skipped); `ResolvePackageLocation` selects the highest matching version by default. |
