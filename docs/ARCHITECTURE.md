...
| 4 | 5 | **Streaming** | ✅ Phase A | Burst-mode streaming input landed 2026-09-16: `XmlStreamingProvider` + `XsltExecutable.TransformStreaming`, record-at-a-time processing in bounded memory (verified at 500k records); VM/XSLT single-pass paths gated on `ISinglePassSequence`. Phase B (streaming accumulators) and Phase C (`streamable="yes"`, §19 posture analysis) pending. |
...
| Streaming | ✅ Phase A | Burst-mode streaming input via `XsltExecutable.TransformStreaming` + `XmlStreamingProvider` (record-at-a-time, bounded memory; forward-only root children with loud errors). Phase B (streaming accumulators) / Phase C (`streamable="yes"`, §19 posture analysis) pending; `xsl:supports-streaming` reports `no` |
