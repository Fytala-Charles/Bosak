...
| 3 | REQ-070 | **Schema awareness / XSD validation** | ✅ Implemented (Phase 4) | User-defined schema simple-type constructor functions (`Q{uri}local#1`), `cast as`, `instance of`, and `ValueMatchesType`/`ApplyFunctionConversion` matching against prefixed user-defined types; schema-validated typed values keep integer kind for integer-derived types and remain date/time typed. Full QT3 sweep 29,929/0/1,892. Remaining documented gaps: `schema-element()`/`schema-attribute()` kind tests (XDocument provider limitation), list/union types, and QName/NOTATION casts. |
