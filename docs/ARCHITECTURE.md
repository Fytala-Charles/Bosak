...
flowchart TB
    ...
    class EXP,COMP,ECTX,FN,MATH,MAP,ARR,XS,VM,FD,SO,ASTO,IRL,LEX,PAR,AST,XV,XN,XSQ,XAV,XD,XMLD,X11,STR current
    class JIT planned
    class CUST external
```

<p class="fytala-figure-caption"><strong>Layer stack.</strong> Dependency direction across the Bosak XPath engine, from the public API down to pluggable node providers; burst-mode streaming input is delivered (Phase A), IL JIT remains planned.</p>
...
**Effective boolean value**: `XdmValue.GetEffectiveBooleanValue()` follows XPath 3.1 rules — empty sequence is `false`, a singleton sequence delegates to its item, a multi-item sequence is `true` only if it contains a node, and a multi-item purely atomic sequence raises `FORG0006`.
...
- **`Xml11Loader` / `Xml11NameCodec`** – because .NET's `System.Xml` stack only accepts XML 1.0 names, the XML 1.1 provider rewrites XML 1.1 declarations, encodes XML 1.1-only name characters as private-use sentinel sequences, and stores them in `XName`. On output, names are decoded again and C0/C1 controls are serialized as numeric references. Prefixed namespace undeclarations (`xmlns:prefix=""`) are preserved via a placeholder URI and a `PrefixedNamespaceUndeclarations` annotation. `Xml11Loader` also scans the document's DTD for `<!ELEMENT>` declarations and annotates elements declared with element-only content (`EMPTY` or a content model without `#PCDATA`) with a `DtdElementOnlyAnnotation`, which the XSLT processor uses for default whitespace stripping.
- **Planned providers** – `XmlDocument` adapter and database-backed nodes.
- **`XmlStreamingProvider`** (`Streaming/`) – burst-mode streaming input: presents an `XmlReader` source as a forward-only document node. The root start-tag is read into a shell document immediately; the root's top-level records are materialized one at a time as detached `XElement`s (reusing `XDocumentNode` internally) as the engine enumerates them, and released once processing moves on. Comments/PIs before the root element are captured into the shell document and surfaced as document-node children before the root. Parent/document chains are re-rooted at the streaming root/document nodes, node identity and document order delegate to the underlying `XObject`s (records are `RegisterTree`'d in arrival order), `StreamingNode` wrappers are cached per underlying node in a `ConditionalWeakTable` (navigation allocates per node, not per access; wrappers are evicted with their record, preserving bounded memory), and second enumerations raise a loud `StreamingException`. See `docs/INTEGRATION.md` §3.2a for the memory contract.
...
|----------|-------|-------------|--------|-------|
| 1 | 4 | **XQuery 3.1** | 🚧 In Progress — Phase 4 | Full core FLWOR (`order by`/`count`/`group by`/`window`), direct and computed constructors with constructor-local namespaces, switch/typeswitch, `validate`, output declarations and serialization, user-defined functions and variables (`declare function`/`declare variable` with function-item coercion and strict declared-type enforcement), library modules (`module namespace`/`import module` with %public/%private visibility), try/catch with named error codes and `err:*` variables, string constructors, ordering features (`ordered`/`unordered`, ordering declarations), spec-correct name tests and constructor in-scope namespaces, namespace undeclaration and declaration static errors (XQST0033/XQST0070, two-phase prolog ordering), inline-function annotations and function-test annotation assertions, character/entity reference validation, map constructors in step position with key disambiguation, `allowing empty` for clauses, computed namespace constructors in content, higher-order function conformance (conversions, absent focus, base-URI capture), duration subtype arithmetic, stable order-by and full switch semantics, `fn:load-xquery-module` (URI resolution with location hints, transitive import closure, external-variable/context-item binding, public-declarations result map; schema imports in the loaded module are propagated to its evaluation context so `validate` and schema-aware expressions work), `declare decimal-format` and `declare boundary-space` (with module-local formats), and the full HTML/XHTML serialization matrix (version-dependent void lists, boolean attributes, raw-text elements, CDATA XML islands, XHTML prefix normalization); QT3 harness routes supported XQuery tests (31,148/0/673, 97.89%). Remaining: none — all runnable tests pass. |
| 2 | — | **XSLT 3.0 packages** | ✅ Implemented | `xsl:package` root parsing, `xsl:use-package` resolution to registered packages, `xsl:expose` static validation and runtime visibility, `xsl:accept` visibility enforcement and runtime checks, accept/override visibility for functions/variables/parameters, diamond-import conflict handling, per-package lazy-global isolation, package-version range matching, `xsl:use-package` in imported modules (`XTSE3008`), library-package absent-focus globals (`XPDY0002`), `xsl:original` for attribute-set overrides, override scope propagation (`xsl:override` variables/functions are visible inside used-package components and `xsl:original(...)` dispatches to the overridden function), and package-scope `fn:function-lookup` (the declaring package's own declarations are returned; `xsl:original` is hidden from lookup) are implemented. `ResolvePackageLocation` uses `PackageVersion.Parse`, `VersionMatches`, and `PackageVersionResolutionStrategy` (`Highest`/`Lowest`). W3C clusters (strict error-code matching): `package` 62/10/0, `accept` 35/15/0, `expose` 41/1/0, `use-package` 49/4/1, `override` 41/58/4, `function-lookup` 8/0/0. Residual: `$xsl:original` variable references, `xsl:original#N` named references, `xsl:original` partial application, and `xsl:call-template name="xsl:original"` are not yet implemented. |
| 3 | REQ-070 | **Schema awareness / XSD validation** | ✅ Implemented (Phase 4) | User-defined schema simple-type constructor functions (`Q{uri}local#1`), `cast as`, `instance of`, and `ValueMatchesType`/`ApplyFunctionConversion` matching against prefixed user-defined types; schema-validated typed values keep integer kind for integer-derived types and remain date/time typed. `schema-element()`/`schema-attribute()` kind tests are evaluated against the compiled schema set, including substitution-group traversal and nillability checks. Lexical timezone offsets are preserved for schema-validated `xs:date`, `xs:time`, `xs:dateTime`, `xs:dateTimeStamp`, and `g*` typed values via `XmlConvert.ToDateTimeOffset`. QName/NOTATION values retain their lexical prefix and schema-type annotation; `xsi:type='xs:ID'` and `xsi:type='xs:IDREF'/'xs:IDREFS'` are honoured for ID/IDREF semantics even without a schema. **REQ-097 (2026-09-22) — XSLT schema-awareness seam H1/H2:** `XsltCompiler.SchemaAware` opts in to schema-aware compilation (default off, bit-identical basic processor); `xsl:import-schema` declarations (inline / `SchemaResolver` / `schema-location` / host `SchemaSet`) compile into one merged `XmlSchemaSet` (import-precedence merge, XTSE0215/XTSE0220) that is folded into `EvaluationContext.SchemaSet` before function-library population, so the REQ-070 machinery (constructors, kind tests) works from stylesheet-declared schemas. Runtime semantics of `validation`/`@type` (hook H4) remain open for the Bosak.Schema commercial track. **REQ-098 (2026-09-22) — XSLT schema-awareness seam H3 (typed construction / annotation API):** `XdmSchemaAnnotator` in `Bosak.XPath.Providers` (`ValidateSubtree` validates an in-memory subtree in place with PSVI attached to the live XObjects — deep-clone + document-order copy-back when the subtree is attached, so node identity is preserved; `Annotate` attaches a host-built `IXmlSchemaInfo`; result type `XdmSubtreeValidationResult` with optional `XmlSchemaValidationException` throw) plus two null-conditional `Action<IXdmNode>` processors on `EvaluationContext` — `ConstructedElementProcessor` fires once per constructed element after content completion, bottom-up (`xsl:element`, literal result elements, `xsl:copy` element results), and `ConstructedDocumentProcessor` fires at result-document boundaries. Core behavior is bit-identical when unset, so hosts can drive validated construction without the engine knowing schemas at construction time. **REQ-099 (2026-09-22) — XSLT schema-awareness seam H4 (validation service + `validation`/`@type` semantics):** `XdmSchemaAnnotator` generalized into a mode-aware validation service (`XdmValidationMode` Strict/Lax/Strip/Preserve, `XdmValidationOptions`; `Validate`/`ValidateAttribute` port the XQuery validate algorithms — lax `xs:anyType` root augmentation, named-type `xsi:type` injection/removal, document-shape checks); TransformEngine reads per-instruction `validation`/`type` (LREs: `xsl:validation`/`xsl:type`; `default-validation` inherited per module) and validates constructed content when a schema set is in scope, raising the XTTE15xx family; PSVI preserved across `xsl:copy`/`xsl:copy-of`; `xsl:strip-type-annotations`/`input-type-annotations="strip"` honored. All four seam hooks H1–H4 are landed; with no schema set in scope the engine is bit-identical. **REQ-100 (2026-09-23) — schema kind tests in match patterns:** `schema-element(N)` / `schema-attribute(N)` in `match` patterns (single, path-step, axis-step, and `@`-attribute positions) are evaluated by the pattern compiler against the schema set captured in the validation context — declaration lookup, substitution-group walk, nilled nillability, `IsDerivedFrom` type-annotation compatibility — mirroring the REQ-070 VM semantics; with no schema set they never match and never throw. **REQ-104 (2026-09-24) — schema kind tests visible in every XPath static context (PA-2):** `CompileOptions.SchemaSet` carries the in-scope compiled schema set into XPath compilation — the parser accepts unprefixed kind-test names in schema-aware mode and the static name-test validator checks the name argument against the set's global declarations (XPST0008 when absent) — and the transform threads its merged set through every XPath compilation (`CompileXPath` options, bare select/catch/copy sites, `xsl:evaluate`, AVTs, patterns), clearing the sweep's XPST0008 cluster; companions: PSVI-typed atomization in variable/parameter coercion (subtype substitution), DateTime-kind g* values in `ValueMatchesType`, and the embedded schema-for-JSON bound by locationless import of the XPath functions namespace. **REQ-105 (2026-09-24) — typed pattern dispatch (PA-3):** the type argument of `element(N,T)`/`element(*,T)`/`attribute(N,T)`/`attribute(*,T)` match patterns is enforced at match time when a schema set is in scope (name resolution + `XmlSchemaType.IsDerivedFrom`, built-in types via the System.Xml.Schema table, nilled elements only against nillable types, unprefixed type names via the xpath-default-namespace); typed forms take default priority 0.25 per XSLT 3.0 §6.4; `schema-element(N)`/`schema-attribute(N)` matching reads the post-compilation `ElementSchemaType`/`AttributeSchemaType`; validated constructed attributes keep their PSVI through sequence-constructor harvesting; validated simple-typed content is whiteSpace-facet normalized (XDM §3.3.2). Without a schema set the type argument stays a no-op (basic processors bit-identical). **REQ-106 (2026-09-24) — NOTATION surface + nested schema imports (PA-3):** casts to NOTATION-derived types annotate `xs:NOTATION` (the namespace-sensitive cast branch dropped the annotation) and enforce the §19.3 stringish-source rule; `xs:QName()` accepts QName-kind input (NOTATION→QName, XPath 3.0); unprefixed type names in `instance of` resolve against no-namespace schema types; grouping keys and `xsl:key` values atomize schema-annotated nodes to their PSVI typed value; `SchemaSetBuilder` eagerly loads locationful nested `xs:import`/`xs:include` targets (`XmlSchemaSet.Compile` fetches nothing with a null resolver in .NET 10). **REQ-107 (2026-09-24) — no-namespace named-type validation + canonical PSVI typed-value forms (PA-3):** named-type validation of no-namespace user types writes unprefixed `xsi:type` on the assessment clone (a generated prefix bound to the empty namespace threw ArgumentException) and drops the clone's default-namespace declaration; `xsl:value-of` atomizes schema-validated nodes via the PSVI typed value (`ConstructValueOfString`/`NodeAtomizedString`, nilled elements contributing no slot); `ConvertSchemaValue` canonicalizes duration lexical forms (`TryCanonicalizeDuration`: months→years fold, zero-component omission, fraction trim, `PT0S`), strips decimal trailing zeros, and keeps `xs:anyURI` lexical verbatim; `HasNoTypedValue` is true only for element-only content (XDM §2.7.2). The type-identity family (as-2002/2101/1806–1809) is tracked as REQ-108. **REQ-108 (2026-09-25) — PSVI user-defined type identity + derivation-aware `instance of` (PA-3):** `XdmValue` carries a second, optional `_userSchemaTypeName` annotation (`Q{uri}local`, public `UserSchemaTypeName`) alongside the untouched built-in base name in `SchemaTypeName`; both `ConvertSchemaValue` paths (XDocumentNode PSVI, VmEngine cast — incl. the QName/NOTATION namespace-sensitive branch) tag user-defined-typed results; `ValueMatchesType`'s user-defined branch requires the value's own type annotation and answers via `IsSchemaTypeSubtype` (exact-QName fallback without a schema set) — castable-but-untyped values are no longer instances of user-defined types, while PSVI/constructor/coercion-typed values match their user type and built-in base; complex-type-with-simple-content values normalize to their simple content base before tagging; annotations survive the bool/float/double/date/time conversion arms; URI promotion still converts-and-loses the user type per XSD 1.0 (no XDM 3.1 anyURI<:string). Schema-aware sweep 10,874/208 → 10,883/199/3,518 (+9 FAIL→PASS, zero pass→fail). |
| 4 | 5 | **Streaming** | ✅ Phases A+B+C+D | Phase A (burst-mode input): `XmlStreamingProvider` + `XsltExecutable.TransformStreaming`, record-at-a-time processing in bounded memory; VM/XSLT single-pass paths gated on `ISinglePassSequence`. Phase B: push-style streaming accumulators. Phase C (2026-09-17): `xsl:supports-streaming` = "yes"; streamable `xsl:source-document`; compile-time XTSE3430 streamability analyzer (`StreamabilityAnalyzer`) per §19 (106/113 corpus static-error cases caught). Phase D (2026-09-17): runtime posture enforcement — fused single-pass eager helpers (EBV, instance-of, treat-as, cardinality), XSLT 3.0 §11.7.3 content-evaluation semantics, `fn:snapshot` grounding of streamed nodes, opt-in record retention (`StreamingLoadOptions.RetainRecords` / `IStreamingDocument.TryEnableReplay`) giving tee/replay for crawling union/except/intersect/fork/map shapes (document-size memory on that path only; the public `TransformStreaming` default stays bounded), streaming DTD/unparsed entities. si-fork residual batch (2026-09-21): group context isolated on template invocation (XTDE1061/1071 in call-template/apply-templates callees), static XTSE3430 for out-of-scope current-group(), generate-id stability across fork prongs, XTDE3365 duplicate map keys inside xsl:fork. Provider batch (2026-09-21): `StreamingNode` wrapper cache (per-node `ConditionalWeakTable`), pre-root comment/PI surfacing on the document axes, `fn:copy-of` provider-agnostic deep-copy guard for streamed nodes, `XsltExecutable.TransformStreamingToString`. su-filter/su-unclassified analyzer batch (2026-09-21): boolean-typed lone-variable filter predicates, positional motionless predicates on striding steps (§19.8.8.9 rule 5), and unclassified-function atomic-param atomization in any argument position (§19.8.5.1). si-iterate XTSE3120 batch (2026-09-21): xsl:break/xsl:next-iteration permitted as the last instruction of xsl:if inside xsl:iterate (§8.4). si-message batch (2026-09-21): conformance harness assert-message matching is non-positional (each assert claims a distinct message; extra messages allowed per the catalog schema). sx-MapExpr map-constructor batch (2026-09-21): `xsl:map` content merges all entries of each map in the content sequence (the spurious single-entry XTTE3365 check is gone; non-map content still raises XTTE3375), and duplicate map-constructor keys inside streamable `xsl:source-document` content raise XTDE3365 (the InStreamingMapContext flag now covers source-document bodies, not only xsl:fork branches). sx-treat/sx-instance-of braced-EQName batch (2026-09-21, parser): braced-URI function calls in step position (`A ! Q{uri}fn(...)`) were misparsed as kind-test steps; `ParseStepExpr` now never treats a `Q{`-prefixed name as a kind test. xml-to-json package-namespace batch (2026-09-21): template-rule `xsl:sequence/@select` is compiled with the instruction's in-scope namespaces (was a bare `XPath31Expression.Compile`), so package-private functions called from used-package template rules resolve against the package's own prefix bindings, not the using stylesheet's. Final full sweep **10,220/55/4,325**; W3C `decl/accumulator` **102/0/5**. |
...
| XSLT 3.0 packages | ✅ Implemented | `xsl:package` root parsing, `xsl:use-package` resolution to registered packages, `xsl:expose` static validation and runtime visibility, `xsl:accept` visibility enforcement and runtime checks, accept/override visibility for functions/variables/parameters, CollectingScope-aware global collection and conflict validation, diamond-import same-name public variables, per-package lazy-global isolation, package-version range matching, `xsl:use-package` in imports/includes (`XTSE3008`), library-package absent-focus globals (`XPDY0002`), `xsl:original` for attribute-set overrides, override scope propagation (`xsl:override` variables/functions visible inside used-package components; `xsl:original(...)` dispatches to the overridden function; `XTSE0770` for duplicate overriding functions), and package-scope `fn:function-lookup` via `EvaluationContext.FunctionLookupInterceptor` (own declarations returned, `xsl:original` hidden) are implemented. W3C clusters (strict error-code matching): `package` 62/10/0, `accept` 35/15/0, `expose` 41/1/0, `use-package` 49/4/1, `override` 41/58/4, `function-lookup` 8/0/0; `ResolvePackageLocation` selects the highest matching version by default. Residual: `$xsl:original` variable references, `xsl:original#N` named references, `xsl:original` partial application, and `xsl:call-template name="xsl:original"` are not yet implemented. |
| Streaming | ✅ Phases A+B+C+D | Burst-mode streaming input via `XsltExecutable.TransformStreaming`/`TransformStreamingToString` + `XmlStreamingProvider` (record-at-a-time, bounded memory by default; forward-only root children with loud errors; pre-root comments/PIs surfaced as document children; per-node wrapper cache) and push-style streaming accumulators (W3C `decl/accumulator` 102/0/5). Phase C: `xsl:supports-streaming` = "yes", streamable `xsl:source-document`, XTSE3430 streamability analysis per §19 (`StreamabilityAnalyzer`, fail-open wrapper in `Stylesheet`). Phase D: analyzer-accepted constructs execute — fused single-pass eager helpers, §11.7.3 content semantics, `fn:snapshot` grounding, opt-in record retention (tee/replay) for crawling multi-operand shapes, streaming DTD support. `fn:copy-of` deep-copies streamed nodes into grounded copies. Group context is isolated across template invocation; duplicate map-constructor keys raise XTDE3365 both inside xsl:fork branches and inside streamable `xsl:source-document` content. Full sweep 10,220/55/4,325 |

Package-version resolution is handled by `XsltFunctionLibrary.ResolvePackageLocation`, which parses registered package versions (`PackageVersion.Parse`) and matches them against the `xsl:use-package` `@package-version` attribute with `VersionMatches`. The supported syntax includes exact versions, wildcard prefixes (`1.*`), hyphen and `to` ranges (`1.0-2.0`, `1.0 to 2.0`), minimum bounds (`1.5+`), comma-separated alternatives, and `*` / empty for any version. When multiple registered versions match, `PackageVersionResolutionStrategy.Highest` selects the highest version (the XSLT 3.0 default); `Lowest` is available for consumers that need deterministic lower-bound selection.
...
| `validate` | ✅ Implemented | Parses as a contextual XQuery keyword; `validate { Expr }`/`validate strict { Expr }`/`validate lax { Expr }` lower to a `ValidateNode` IR opcode. `validate lax` with no schema returns the operand unchanged; `validate strict`/plain `validate` without a schema raises `XQST0075`; invalid operands raise `XQTY0030`; validation failure raises `XQDY0027`.
...
src/
  ...
  Bosak.XPath.Providers/    XDocument adapter (XDocumentNode), XML 1.1 loader/codec, burst-mode streaming input (XmlStreamingProvider); XmlDocument adapter planned
  ...
  Bosak.LanguageServer/   LSP server for XPath / XSLT / XQuery diagnostics, completions, hover, go-to-definition, document symbols, semantic tokens, code actions, code lens for XPath/XQuery results and XSLT transformations, and an XSLT initial-template runner (OmniSharp 0.19.9)
