...
- **XQuery 3.1 (Phase 2 complete)** — `order by`, `count`, `group by`, and `window` clauses implemented with tuple-based VM lowering; constructors and modules remain
...
| **XQuery** | `Bosak.XQuery` | `XQueryCompiler`, `XQueryExecutable`, `XQueryParser`, `XQueryStaticContext`; Phase 2 complete (`order by` + `count` + `group by` + `window` implemented; constructors/modules remain) |
...
| 3 | XQuery 3.1 — prolog parser, static context, prolog-less queries, full core FLWOR | 🚧 Phase 2 complete (all core FLWOR clauses implemented); Phase 3 constructors next |
