...
- **XSLT streaming** — Phase A landed 2026-09-16: burst-mode streaming input via `XmlStreamingProvider` (Providers) and `XsltExecutable.TransformStreaming` — record-at-a-time processing of very large documents in bounded memory, verified at 500k records with single-digit MB live growth. `xsl:supports-streaming` still reports `no`; remaining phases: streaming accumulators (B) and `streamable="yes"` with §19 posture analysis (C)
