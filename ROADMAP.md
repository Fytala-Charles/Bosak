...
- **XSLT streaming** — Phases A+B landed 2026-09-16: burst-mode streaming input via `XmlStreamingProvider` and `XsltExecutable.TransformStreaming` (record-at-a-time processing in bounded memory, verified at 500k records), plus push-style streaming accumulators (values computed per record as the stream arrives; W3C `decl/accumulator` set at 93/0/14 — 100% of runnable). `xsl:supports-streaming` still reports `no`; remaining: `streamable="yes"` with §19 posture analysis and the `strm/` sets (Phase C)
